...
timeSharingOption = {
    ...
    tooltip: {
        ...
        formatter: function (params) {
            ...
            let turnOverRate = timeSharingOption.series[5].turnOverRates[index];
            let changeRate = timeSharingOption.series[0].changeRates[index];
            ...
            for (let i = 0; i < params.length; i++) {
                ...
                if (params[i].seriesName === '成交量') {
                    ...
                    if (i === 0)
                        res += '<br/>';
                } else if (params[i].seriesName === '分时图') {
                    res = res + '分时图' + '<br/>开盘价: ' + value[0] + '  收盘价: ' + value[1] + '<br/>最低价: ' + value[2] + ' 最高价: ' + value[3] + '<br/>涨跌幅: ' + Math.round(changeRate * 100) / 100 + '%<br/>';
                } else {
                    if (value !== '-')
                        value = Math.round(params[i].value * 100) / 100;
                    res = res + '<span style="display:inline-block;margin-right:5px;border-radius:10px;width:9px;height:9px;background-color:' + params[i].color + '"></span>' + params[i].seriesName + ': ' + value;
